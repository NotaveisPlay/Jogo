...
// ==========================================
// GERADOR INFINITO DE CÁLCULOS MULTIPLAYER
// ==========================================
function sup(str) { return str.replace(/\^2/g, "²"); }

function gerarQuestao() {
  const n = Math.floor(Math.random() * 9) + 1; // Gera números de 1 a 9
  ...
  if (tipo === 0) {
    return { 
      prompt: sup(`(x + ${n})^2`), 
      answer: `x² + ${2*n}x + ${n*n}`, 
      hint: "Primeiro ao quadrado, mais duas vezes o primeiro pelo segundo, mais o segundo ao quadrado." 
    };
  } else if (tipo === 1) {
    return { 
      prompt: sup(`(x - ${n})^2`), 
      answer: `x² - ${2*n}x + ${n*n}`, 
      hint: "O sinal do termo do meio fica negativo." 
    };
  } else {
    return { 
      prompt: sup(`(x + ${n})(x - ${n})`), 
      answer: `x² - ${n*n}`, 
      hint: "Soma pela diferença: quadrado do primeiro menos o quadrado do segundo." 
    };
  }
}
...
// Lógica para conferir se a resposta digitada tá certa
...
function addPolys(a, b, factor = 1) {
  const r = { ...a }; for (const k in b) r[k] = (r[k] || 0) + factor * b[k];
  Object.keys(r).forEach(k => { if (r[k] === 0) delete r[k]; }); return r;
}
function multiplyPolys(a, b) {
  const result = {};
  for (const k1 in a) {
    for (const k2 in b) {
      const c = a[k1] * b[k2]; const map = {};
      (k1 + "," + k2).split(",").filter(Boolean).forEach(part => { const [v,p] = part.split("^"); map[v] = (map[v] || 0) + Number(p); });
...
function parsePolynomial(expr) {
  ...
  function parseExpression() { let left = parseTerm(); while (i < tokens.length && (tokens[i] === "+" || tokens[i] === "-")) left = addPolys(left, parseTerm(), tokens[i++] === "+" ? 1 : -1); return left; }
  function parseTerm() { let left = parseFactor(); while (i < tokens.length && tokens[i] === "*") { i++; left = multiplyPolys(left, parseFactor()); } return left; }
  function parseFactor() {
    if (tokens[i] === "+") { i++; return parseFactor(); } if (tokens[i] === "-") { i++; const f = parseFactor(); return Object.fromEntries(Object.entries(f).map(([k,v]) => [k, -v])); }
    let base; if (tokens[i] === "(") { i++; base = parseExpression(); if (tokens[i] === ")") i++; } else if (/^\d+$/.test(tokens[i] || "")) base = { "": Number(tokens[i++]) }; else if (/^[a-z]$/.test(tokens[i] || "")) base = { [tokens[i++] + "^1"]: 1 }; else base = { "": 0 };
    if (tokens[i] === "^") { i++; const exp = Number(tokens[i++] || 1); let result = { "": 1 }; for (let k = 0; k < exp; k++) result = multiplyPolys(result, base); return result; }
    return base;
  }
...
function answerIsCorrect(input, expected) {
  ...
  try { return canonical(parsePolynomial(input)) === canonical(parsePolynomial(expected)); } catch { return false; }
}
